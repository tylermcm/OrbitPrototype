...
public:
	...
	static Player* create(double posX, double posY, double velX, double velY, double angle)
	{
		...
		auto newPlayer = new Player(initialPos, velX, velY, angle);
	...
	{
		Position initialPos;
		initialPos.setPixelsX(posX);
		initialPos.setPixelsY(posY);
		return initialPos;
	}
	...
	Projectile* shoot()
	{
		...
		double additionalVelocity = 9000 + physics.
			computeTotalComponent(physics.getVelocityX(), physics.getVelocityY());

		auto newProjectile = new Projectile(projectileStart, shootAngle, additionalVelocity);
